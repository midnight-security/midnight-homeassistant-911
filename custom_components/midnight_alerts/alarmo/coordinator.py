"""Alarmo coordinator, storage wiring, and services - merged from the
upstream Alarmo integration (github.com/nielsfaber/alarmo).

Platform forwarding (alarm_control_panel) is handled by the parent
midnight_alerts integration; this module owns everything else Alarmo needs:
its coordinator, device registration, panel/card/websocket registration,
...
from ..const import DOMAIN as MIDNIGHT_DOMAIN
...
async def async_setup_alarmo(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Set up the merged Alarmo coordinator, device, panel, and services."""
...
async def async_unload_alarmo(hass: HomeAssistant) -> None:
    """Unload the merged Alarmo coordinator and panel."""
    async_unregister_panel(hass)
    coordinator = hass.data[const.DOMAIN]["coordinator"]
    await coordinator.async_unload()


async def async_remove_alarmo(hass: HomeAssistant) -> None:
    """Wipe Alarmo's stored configuration entirely."""
    coordinator = hass.data[const.DOMAIN]["coordinator"]
