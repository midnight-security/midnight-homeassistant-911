...
TRIGGER = (
    "custom_components.midnight_alerts.api.MidnightAlertsApiClient.async_trigger_alert"
)
...
async def test_button_press_failure_is_logged_not_raised(hass, caplog):
    ...
    with patch(TRIGGER, new=AsyncMock(side_effect=MidnightAlertsApiError("boom"))):
...
async def test_button_press_without_api_key_is_a_local_noop(hass, caplog):
    ...
    with patch(
        TRIGGER, new=AsyncMock(side_effect=AssertionError("should not be called"))
    ):
        await hass.services.async_call(
            "button",
            "press",
